...
npx.v = 3
fz.LED(0.2, npx, [0, 0, npx.v], [0, npx.v, 0], 10)
i2c, rtc, rb = fz.ini_sp_(npx, iri_i, cal_i)
#    0-4. Watchdog
# wdt.timeout = T1 * 2
# wdt.timeout = T1 * 2
# wdt.mode = wdm.RESET
...
if iri_i >= 1 :
    fz.LED(.25, npx, [0, 0, npx.v], [0, npx.v, npx.v], 10)
    fz.prt(Fil_Log, "\n\nIridium SBD...\t{:6.1f} [s]".format(t.monotonic()-ts))
    gc.collect()
    fz.prt(Fil_Log, "\n\tgc.mem_free() = {:6.1f}".format(gc.mem_free()))
    Nr = fz.iri(iri_i, rb, Hs, Fp, Pzz, lat, lon, temp, pres, hmd, Slev, npx, Fil_Log)
    # fz.prt(Fil_Log, "\n\tTrasfrer data =\t{}".format(rb_sent))
    fz.prt(Fil_Log, "\n\t\t The number of retry : {:2d}".format(Nr))
